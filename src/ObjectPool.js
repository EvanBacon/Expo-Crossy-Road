...
import getShader from './loader/getShader';
import WorldPieces from './loader/WorldPieces';
import CommonWorldPieces from './loader/CommonWorldPieces';
import * as AssetLoader from './loader/AssetLoader';
...
export function GetRandomWorldPieceVariation({ currentWorldPieces, CurrentRow }, worldPiece) {
    let worldPieces = currentWorldPieces;
    if (worldPiece === 'coin' || worldPiece === 'red-coin') {
        worldPieces = CommonWorldPieces;
    }
    ...
    for (var i = 0; i < worldPieces[worldPiece].Variations.length; i++) {
        if (CurrentRow >= worldPieces[worldPiece].Variations[i].scoreRequired) {
    ...
    let currentTotal = 0;
    for (var i = 0; i < worldPieces[worldPiece].Variations.length; i++) {
        if (CurrentRow + 2 >= worldPieces[worldPiece].Variations[i].scoreRequired) {
...
class ObjectPooler {
    ...
    /**
     * 
     * @param { scene: THREE.Scene, CurrentRow: number,  currentWorldPieces: any, worlds: { [key: string]: any } } world 
     */
    constructor(world) {
        this.world = world;
    }
    ...
    GetPieceFrames({ name }) {
        const modelName = `${name.replace(/-/g, '_')}_optimised`;
        for (let i = 0; i < Object.keys(this.world.worlds).length; ++i) {
            const worldObject = AssetLoader.loadedAssets[`models/${Object.keys(this.world.worlds)[i]}-world.json`];
            if (worldObject && worldObject.models && worldObject.models[modelName]) {
                return [worldObject.models[modelName]];
            }
        }
        return [AssetLoader.loadedAssets['models/common-world.json'].models[modelName]];
    }
    ...
    GetItemVariation(type, variation, forceNew) {
        ...
        if (this.items[type + variation.name] == null || this.items[type + variation.name].length == 0 || forceNew) {
            if (!variation.frames) {
                for (let i = 0; i < this.world.currentWorldPieces[type].Variations.length; ++i) {
                    if (this.world.currentWorldPieces[type].Variations[i].name === variation.name) {
                        this.world.currentWorldPieces[type].Variations[i].frames = variation.frames = this.GetPieceFrames(variation);
                        break;
                    }
                }
            }
    ...
    GetItemOfType(type) {
        let worldPieces = this.world.currentWorldPieces;
